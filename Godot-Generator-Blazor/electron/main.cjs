...
// ── App menu ──────────────────────────────────────────────────────────────────
//
// The cross-platform menu structure lives in menu.yaml as `DeclarativeMenuItem[]`.
...
/**
 * Loads `menu.yaml`, builds the typed action registry, and sets the application
...
async function buildAndSetMenuAsync() {
  ...
  // ── Action registry ─────────────────────────────────────────────────────────
  // Maps each actionId declared in menu.yaml to a typed ActionDescriptor.
  ...
  const spec = await loadMenuSpecFromFile(path.join(__dirname, 'menu.yaml'));
  ...
  const template = [
    ...
    // Cross-platform items resolved from menu.yaml.
    ...crossPlatform,
